...
#include <linux/leds.h> //define led_classdev structure
...
static void brightness_set_4412(struct led_classdev *led_cdev, enum led_brightness brightness)
{
	...
	if(brightness != LED_OFF)
		gpio_set_value(dev->gpio, 0); //turn on
	else
		gpio_set_value(dev->gpio, 1); //turn off
}
